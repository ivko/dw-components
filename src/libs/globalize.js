...
(function() {
	...
	expandNumber = function (number, precision, formatInfo, valueIsNumber) {
	    // if valueIsNumber value is not passed - by default should be true
	    valueIsNumber === false ? valueIsNumber : valueIsNumber = true;
	...
	formatNumber = function (value, format, culture) {
	    // check the type of the given value, could be:
        // number or string
		...
		var nf = culture.numberFormat,
			number = valueIsNumber ? Math.abs( value ) : value.replace((/-/g), ""),
			precision = -1,
			pattern;
		...
		switch ( current ) {
			...
			case "P":
				...
				pattern = (value < 0 || value.toString().indexOf("-") >= 0) ? formatInfo.pattern[0] : (formatInfo.pattern[1] || "n");
				if ( precision === -1 ) precision = formatInfo.decimals;
				number = valueIsNumber ? expandNumber(number * (current === "P" ? 100 : 1), precision, formatInfo, valueIsNumber) : expandNumber(number, precision, formatInfo, valueIsNumber);
				break;
			default:
