 ...
 $(selector).datetimeEntry('setDatetime', null) */
        ...
        _doKeyDown: function (event) {
            ...
            switch (event.keyCode) {
                case 8:
                    var input = event.target;
                    if (input.selectionStart <= plugin._fieldLength(inst, 0, inst.options.dateFormat))  //check me: date format here includes separators
                        inst._field = 0;
                    else if (input.selectionStart <= plugin._fieldLength(inst, 0, inst.options.dateFormat) + plugin._fieldLength(inst, 1, inst.options.dateFormat) + 1)
                        inst._field = 1;
                    return true;
                    break;
                ...
                case 46: plugin._setValue(inst, ''); // Clear date/time on delete
                    inst._field = 0; // First field on home
                    break;
                default: return true;
            }
            return false;
        ...
        _handleKeyPress: function (inst, chr) {
            ...
            if ((inst.options.datetimeSeparators + sep).indexOf(chr) > -1) {
                ...
                if ((field == 'd' || field == 'w' || field == 'W') && (day > this._getDaysInMonth(year, month - 1)))
                    month++;
                if ((field == 'm' || field == 'n' || field == 'N') && (day > this._getDaysInMonth(year, month - 1)))
                    day = this._getDaysInMonth(year, month - 1);
                ...
                var skipToNext = ((field != 'Y') && (inst._lastChr.length > 0)) ||
                                 ((field == 'Y') && (inst._lastChr.length > 2));  //last here is not year -> check me
