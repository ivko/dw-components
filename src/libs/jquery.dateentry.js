 ...
 $(selector).dateEntry('setDate', null) */
		...
		_doKeyDown: function(event) {
			...
			switch (event.keyCode) {
			    case 8:
			        var input = event.target;
			        if (input.selectionStart <= plugin._fieldLength(inst, 0, inst.options.dateFormat))
			            inst._field = 0;
			    ...
			    case 46: plugin._setValue(inst, ''); // Clear date on delete
		...
		_handleKeyPress: function(inst, chr) {
			if (inst.options.dateFormat.substring(3).indexOf(chr) > -1) {
				...
				var skipToNext = ((field != 'y') && (inst._lastChr.length > 0)) ||
                                 ((field == 'y') && (inst._lastChr.length > 2));
